...
TH2D* hDeltaTac[2*nRomanPots][nIterations];
double tacAvrg[2*nRomanPots][100][nIterations];
...
void slewing()
{
	TString input = "/home/truhlar/Desktop/STAR/CEP/Analysis/Data/Ana1102.root";
	TString output = "/home/truhlar/Desktop/STAR/CEP/Analysis/Outputs/SlewNew.root";
    ...
    for (int iCh = 0; iCh < 2*nRomanPots; ++iCh)
        for (int i = 0; i < 100; ++i)
    ...
    for (int iter = 0; iter < nIterations; ++iter)
    {
        ...
        for (int iCh = 0; iCh < 2*nRomanPots; ++iCh)
        {
            ...
            TF1 *gaus = new TF1("gaus","gaus(0)",hDeltaTac[iCh][iter]->GetMean(2) - 200, hDeltaTac[iCh][iter]->GetMean(2) + 200);
            ...
            {
                tacAvrg[iCh][iBin][iter] = h2_1->GetBinContent(iBin+1);
...
void Init()
{
    ...
    for (int iter = 0; iter < nIterations; ++iter)
    {
        ...
        for (int i = 0; i < 2*nRomanPots; ++i)
        {                      
                hDeltaTac[i][iter] = new TH2D("DeltaTac_" + rpNames[i/2] + Form("_Channel_%i_%i,",i%2,iter), "DeltaTac_" + rpNames[i/2] + Form("Channel_%i",i%2), 100,0,800,100,-1000,1000);                                           
        }
    }
}
